...
export const ShopCart = ({
  ...
  const createOrder = async () => {
    ...
    console.log(responseRes, "cartFoods-cartItemsTotalPrice");
    if (!responseRes.ok) {
  ...
  return (
    <Drawer direction="right" open={cartOpen} onOpenChange={setCartOpen}>
      <DrawerTrigger asChild onClick={reloadFoods}>
        <Button variant={"outline"} className="size-9 rounded-full">
          <LuShoppingCart size={16} className="text-secondary-foreground" />
        </Button>
      </DrawerTrigger>

      <DrawerContent className="sm:max-w-[535px] p-8 gap-6 bg-neutral-700 text-primary-foreground border-none rounded-[20px]">
